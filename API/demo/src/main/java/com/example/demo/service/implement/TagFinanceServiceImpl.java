...
public class TagFinanceServiceImpl implements TagFinanceService {
    ...
    public BaseResponse createTagFinance(String name, String description) {
        return new BaseResponse(MessageResponse.INF_TAG,
                new TagFinanceResponse(tagFinanceDAO.createTagFinance(name, description)));
    }


    @Override
    public BaseResponse updateTagFinance(TagfinanceRequest tagfinanceRequest, @PathVariable(name = "id") int Id) {
        TagFinance tagfinanceExisted = tagFinanceDAO.getTagFinanceById(Id);
        TagFinance tagfinanceUpdate = new TagFinance(tagfinanceExisted, tagfinanceRequest);
        tagFinanceDAO.updateTagFinance(tagfinanceUpdate);
